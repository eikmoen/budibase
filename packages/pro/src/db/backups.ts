...
export function generateWorkspaceBackupID(
  ...
  return `${WORKSPACE_BACKUP_PREFIX}${workspaceId}${dbCore.SEPARATOR}${timestamp}`
}
...
export async function updateWorkspaceBackupMetadata(
  ...
  const db = tenancy.getGlobalDB()
  const metadata = await db.get<WorkspaceBackup>(backupId)
  metadata.name = name
...
export async function getWorkspaceBackupMetadata(backupId: string) {
  const db = tenancy.getGlobalDB()
  return await db.get<WorkspaceBackup>(backupId)
}
